...
namespace JieNor.Megi.DataRepository.SEC
{
    public class SECUserRepository : DataServiceT<SECUserModel>
    {
        ...
        //public SECUserModel GetList(string email)
        //{
        //	string sQLString = "select * from t_sec_user where MEmailAddress=@MEmailAddress ORDER BY MIsTemp";
        //	MySqlParameter[] cmdParms = new MySqlParameter[1]
        //	{
        ...
        //public List<SECUserModel> GetUserListAll(MContext ctx, SqlWhere filter)
        //{
        //    StringBuilder stringBuilder = new StringBuilder();
        ...
        public OperationResult UpdateStatus(MContext ctx, string mItmeId, int status)
        {
            ...
            try
            {
                string sql = "update t_sec_user set MIsActive=" + status + " where MItemID ='" + mItmeId + "';";
                //DynamicDbHelperMySQL dynamicDbHelperMySQL = new DynamicDbHelperMySQL(ctx);
                ...
                {
                    operationResult.Success = false;
                    operationResult.Message = "¸üÐÂÊ§°Ü";
