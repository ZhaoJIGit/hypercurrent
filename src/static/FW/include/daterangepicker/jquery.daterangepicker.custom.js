        ...
        _generateHTML: function (inst) {
            ...
            for (var row = 0; row < numMonths[0]; row++) {
                ...
                for (var col = 0; col < numMonths[1]; col++) {
                    ...
                    //felson
                    leadDays = leadDays == 0 ? 7 : leadDays;
                    var numRows = (isMultiMonth ? 6 : Math.ceil((leadDays + daysInMonth - 1) / 7));
                    var printDate = this._daylightSavingAdjust(new Date(drawYear, drawMonth, 1).addDays(1 - leadDays));
                    for (var dRow = 0; dRow < numRows; dRow++) {
                        ...
                        for (var dow = 0; dow < 7; dow++) {
                            ...
                            tbody += '<td class="'
                                ...
                                + (unselectable ? "" : " onclick=\"DP_jQuery.datepicker._selectDay('#" + inst.id + "'," + drawMonth + "," + drawYear + ', this);return false;"')
                                + ">"
                                + (otherMonth ?
                                    (showOtherMonths ? printDate.getDate() : "&#xa0;")
                                    : (unselectable ?
                                        '<span class="ui-state-default">' + printDate.getDate() + "</span>"
                                        : '<a class="ui-state-default' + (printDate.getTime() == today.getTime() ?
                                            " ui-state-highlight" : "")
                                            + (printDate.getTime() >= currentDate.getTime() && printDate.getTime() <= endDate.getTime() ?
                                                " ui-state-active" : "") + '" href="#">' + printDate.getDate() + "</a>"))
                                + "</td>";
