...
jQuery.fn.daterangepicker = function (settings) {
    ...
    var options = jQuery.extend({
        ...
        // date formatting. Available formats: http://docs.jquery.com/UI/Datepicker/%24.datepicker.formatDate
        closeOnSelect: false,
    ...
    var datepickerOptions = {
        onSelect: function () {
            ...
            if (rangeInput.length == 2) {
                if (rangeInput.eq(0).is("input")) {
                    rangeInput.eq(0).val(rangeA);
                }
                else {
                    rangeInput.eq(0).text(rangeA);
                }
                if (rangeInput.eq(1).is("input")) {
                    rangeInput.eq(1).val(rangeA);
                }
                else {
                    rangeInput.eq(1).text(rangeA);
                }

            }
            else {
                if (rangeInput.is("input")) {
                    rangeInput.val((rangeA != rangeB) ? rangeA + ' ' + options.rangeSplitter + ' ' + rangeB : rangeA);
                }
                else {
                    rangeInput.text((rangeA != rangeB) ? rangeA + ' ' + options.rangeSplitter + ' ' + rangeB : rangeA);
                }
            }
            ...
            var obj = window.currentDPTarget;

            if ($(obj).attr("triggerOnChange") == "1" || $(obj).is("li")) {
                options.onChange();
            }
    ...
    var doneBtn = jQuery('<input type="button" class="btnDone ui-state-default ui-corner-all" value="' + options.doneButtonText + '"/>')
    .click(function () {
        var a = rp.find('.ui-datepicker-current-day');
        for (var i = 0; i < a.length ; i++) {
            $(this).attr("triggerOnChange", (i == (a.length - 1)) ? "1" : "0");
    ...
    var rpPresets = (function () {
        ...
        jQuery.each(options.presetRanges,
        function () {
            jQuery('<li class="ui-daterangepicker-' + this.text.replace(/ /g, '') + ' ui-corner-all"><a href="#" style="margin-left:21px">' + this.text + '</a></li>')
        ...
        var x = 0;
        jQuery.each(options.presets,
        function (key, value) {
            jQuery('<li class="ui-daterangepicker-' + key + ' preset_' + x + ' ui-helper-clearfix ui-corner-all"><span class="ui-icon ui-icon-triangle-1-w"></span><a href="#">' + value + '</a></li>')
            .appendTo(ul);
            x++;
    ...
    jQuery.fn.clickActions = function (rp, rpPickers, doneBtn) {
        ...
        if (jQuery(this).is('.ui-daterangepicker-specificDate')) {
            ...
            //custom date range
            doneBtn.hide();
            rp.find('.range-start, .range-end').hide(400,
            function () {
                rpPickers.hide();
            ...
            /*
            update by 锦友 2018-06-26
            修复BUG 23120
            原来代码会促发2次最近时间的选择事件，执行2次查询
            截止时间还没赋值就已经执行了一次查询把今天的时间包括在里面了
            现在的解决方案是赋值和绑定事件分开
            */
            ...
            rp.find('.range-end').find('.ui-datepicker-current-day').trigger('click');
            window.currentDPTarget = null;
        }
        return false;
    ...
    if (options.posX) {
        rp.parent().css('right', options.posX);
