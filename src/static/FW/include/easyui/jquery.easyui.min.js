...
(function ($) {
    ...
    function _116(_117, ul, _118, _119) {
        ...
        var _11c = opts.loader.call(_117, _118, function (data) {
            ...
            //对整个返回的字符串进行HtmlDecode chenpan
            if (data) {
                data = mText.jsonDecode(data, _.htmlDecode);
            }
        ...
        render: function (_19d, ul, data) {
            ...
            function _19f(_1a0, _1a1) {
                ...
                for (var i = 0; i < _1a1.length; i++) {
                    ...
                    cc.push("<span class=\"tree-title\">" + mText.encode(opts.formatter.call(_19d, item)) + "</span>");
                    cc.push("</div>");
                    if (item.children && item.children.length) {
...
(function ($) {
    ...
    function _1d9(_1da) {
        ...
        if (_1db) {
            ...
            if (opts._title) {
                //$(_1da).attr("title", opts._title).attr("etitle", opts._title);
                $(_1da).attr("etitle", opts._title);
            }
    ...
    $.fn.tooltip.parseOptions = function (_1e0) {
		...
		$.parser.parseOptions(_1e0, ["position", "showEvent", "hideEvent", "content", {
		    ...
		    //_title: t.attr("title") || t.attr("etitle")
		    _title: t.attr("etitle")
		});
        //t.attr("title", "").attr("etitle", "");
        t.attr("etitle", "");
        if (!opts.content) {
    ...
    function _2b3(_2b4, pp, _2b5) {
        ...
        t.bind("click", function () {
            ...
            if (pp.panel("options").collapsed) {
                ...
                //felson 注释掉的，意思是用户点击的时候只展开，不做收起，需要点击其他地方的时候才做收起
                //_2be(_2b4, _2b9);
            }
            return false;
        });
...
(function ($) {
    ...
    function _3e7(_3e8, _3e9) {
        ...
        if (_3e9) {
            ...
            //felson 2018.6.14
            if (opts.showMenuOnClick === true) {
                btn.unbind(".menubutton").bind("click.menubutton", function () {
                    _3eb(_3e8)
                    return false;
                });
            }
...
(function ($) {
    ...
    function _477(_478, _479) {
        ...
        t._outerWidth(opts.width)._outerHeight(opts.height < 27 ? 27 : opts.height);
        t.css("line-height", (t.height() < 25 ? 25 : t.height()) + "px");
        t.insertAfter(_47a);
        _47a.remove();
    };
    function _476(_47b) {
    ...
    function _475(_47c, _47d) {
        ...
        $(_47c).validatebox && $(_47c).validatebox("validate");
        if (_47f != _47d) {
    ...
    function _480(_481) {
        ...
        $(_481).unbind(".numberbox").bind("keypress.numberbox", function (e) {
        ...
        }).bind("keyup.numberbox", function (l) {
            //为了解决，tab 和 shift+tab来回切换的问题，加上了如下的方法，希望不会有什么问题
            if (l.keyCode == 9 || l.which == 9 || l.keyCode == 16 || l.which == 16) {
                return true;
            }
            ...
            //如果有允许的值
            if (allowChar && allowChar.length > 0 && !!this.value) {

                var pass = false;
                allowChar = allowChar.distinct();
                for (var x = 0; x < allowChar.length; x++) {

                    var charPass = true;
                    for (var y = 0 ; y < this.value.length ; y++) {
                        charPass &= (this.value[y] === allowChar[x]);
                    }
            ...
            while (!$.isNumeric(v) && v && v != '-' && v != '+' && v.length > 0) {
                v = v.substring(0, v.length - 1);
            }
            if (v != $(this)[0].value) {
                $(this).val(v);
                p = p > v.length ? v.length : p;
                ms(this, p);
            }
            return true;
        });
    ...
    $.fn.validatebox.defaults, {
                ...
                if (c == opts.groupSeparator) {
                    ...
                    if ((e.which >= 48 && e.which <= 57 && e.ctrlKey == false && e.shiftKey == false) || e.which == 0 || e.which == 8) {
                        ...
                        if (e.ctrlKey == true && (e.which == 99 || e.which == 118)) {
                            ...
                            //兼容其他使用Shift+数字符号的输入法
                        else if (e.shiftKey == true && e.which >= 48 && e.which <= 57) {
                            return true;
                        }
                            //如果是输入的等于号并且当前值未空字符串
                        else if (e.which == 61 && $(this).data("allowChar") && $(this).data("allowChar").contains("=")) {
                            return true;
                        }
                        else {
                            return false;
        ...
        parser: function (s) {
            ...
            if (opts.minPrecision != undefined) {
                precision = opts.minPrecision;
                if (s != null && s != "" && String(s).indexOf(".") > -1) {
                    s = s.replace(new RegExp(",", "g"), "");
                    var arrNumber = s.split(".");
                    if (arrNumber.length == 1 || arrNumber[1].length <= opts.minPrecision) {
                        precision = opts.minPrecision;
                    } else if (arrNumber[1].length < opts.precision) {
            ...
            if (isNaN(val)) {
                ...
                if (typeof (opts.min) == "number" && val < opts.min) {
                    val = opts.min.toFixed(precision);
                } else if (typeof (opts.max) == "number" && val > opts.max) {
                    val = opts.max.toFixed(precision);
                }
                else if (opts.keepText === true) {
                    return s;
                }
            }
            return val;
...
(function ($) {
    ...
    function show(_4ae) {
        ...
        $(_4ae).find(".calendar-title span").html(!opts.current ? "" : (opts.months[opts.month - 1] + " " + opts.year));
...
(function ($) {
    ...
    function _519(_51a) {
        ...
        _521.height(hh);
        _522.height(hh);
        _51f.add(_520);//felson ._outerHeight(hh);
        if (opts.height != "auto") {
        ...
        dc.body1.add(dc.body2).unbind().bind("mouseover", function (e) {
            ...
            if (tt.parent().hasClass("datagrid-cell-check")) {
                ...
                if (td.length) {
                    ...
                    if (opts.onClickCell.call(_55a, _567, _568, row[_568], e) === false) return;
            ...
            if (($(tt).is("input[type='text']") || $(tt).is("input[type='password']") || $(tt).is("textarea")) && !$(tt).hasClass("click-no-select")) { $(tt).trigger("click.click2select") }
            e.stopPropagation();
        }).bind("dblclick", function (e) {
            var tt = $(e.target);
            var tr = tt.closest("tr.datagrid-row");
    ...
    function _endEdit(_5fe, _5ff, _600) {
        ...
        if (!_600) {
            ...
            tr.find("div.datagrid-editable").each(function () {
                ...
                //获取这一样的整个参数col，包括名称啊，formatter啊，宽度等等
            ...
            for (var i = 0; i < _columnFields.length; i++) {
                ...
                if (col) {
                    ...
                    if (col.checkbox) {
                        ...
                        //如果有formatter，则调用formatter，如果没有就直接显示值
                        if (col.formatter) {
                            //
                        ...
                        else if (_fieldTextValue != undefined) {
                            //如果是下拉框类型的，则需要进行特别的处理
...
(function ($) {
    ...
    function _73d(_73e) {
        ...
        opts, {
            ...
            onClickRow: function (_740, row, e) {
                if (_73c != this) {
                    _741(_73c);
                    _73c = this;
                }
                ...
                opts.onClickRow.call(_73e, _740, row, e);
            },
            loadFilter: function (data) {
                _741(this);
...
(function ($) {
    function _771(_772) {
        ...
        opts, {
            ...
            onAfterEdit: function (_778, row, _779) {
                return opts.onAfterEdit.call(_772, row, _779);
            },
            onCancelEdit: function (_77a, row) {
                return opts.onCancelEdit.call(_772, row);
            },
            onSelect: function (_77b) {
                return opts.onSelect.call(_772, find(_772, _77b));
            },
            onUnselect: function (_77c) {
                return opts.onUnselect.call(_772, find(_772, _77c));
            },
            onCheck: function (_77d) {
                return opts.onCheck.call(_772, find(_772, _77d));
            },
            onUncheck: function (_77e) {
                return opts.onUncheck.call(_772, find(_772, _77e));
            },
            onClickRow: function (_77f) {
                return opts.onClickRow.call(_772, find(_772, _77f));
            },
            onDblClickRow: function (_780) {
                return opts.onDblClickRow.call(_772, find(_772, _780));
            },
            onClickCell: function (_781, _782) {
                return opts.onClickCell.call(_772, _782, find(_772, _781));
            },
            onDblClickCell: function (_783, _784) {
                return opts.onDblClickCell.call(_772, _784, find(_772, _783));
            },
            onRowContextMenu: function (e, _785) {
                return opts.onContextMenu.call(_772, e, find(_772, _785));
            }
        }));
        if (!opts.columns) {
    ...
    function _795(_796) {
        ...
        dc.body1.add(dc.body2).bind("mouseover", function (e) {
            ...
            if (tt.hasClass("tree-hit") || tt.hasClass("tree-folder")) {
        ...
        render: function (_7f9, _7fa, _7fb) {
            ...
            function _800(_801, _802, _803) {
                ...
                for (var i = 0; i < _803.length; i++) {
                    ...
                    var css = opts.rowStyler ? opts.rowStyler.call(_7f9, row, i) : "";
                    var _805 = "";
                    var _806 = "";
                    if (typeof css == "string") {
                        _806 = css;
...
(function ($) {
    function _83e(_83f, _840) {
        ...
        _842.appendTo("body");
        var _844 = _842.find("input.combo-text").length == 0 ? _842.find("textarea.combo-text") : _842.find("input.combo-text");
        ...
        _844.css({
            height: (_842.height() < 25 ? 25 : _842.height()) + "px",
            lineHeight: (_842.height() < 25 ? 25 : _842.height()) + "px"
        });
        _845._outerHeight(_842.height());
        _843.panel("resize", {
            width: (opts.panelWidth ? ($.isFunction(opts.panelWidth) ? opts.panelWidth() : opts.panelWidth) : _842.outerWidth()),
    ...
    function init(_847) {
        ...
        //span列表
        var spanText = "<span class=\"combo\">";
        //如果本身是textarea的生成的下拉框也是textarea
        if (!$(_847).is("textarea")) {
            spanText += "<input type=\"text\" class=\"combo-text\" autocomplete=\"off\">" + "<span><span class=\"combo-arrow\"></span></span>" + "<input type=\"hidden\" class=\"combo-value\">" + "</span>";
        }
        else {
            spanText += "<textarea type=\"text\" class=\"combo-text combo-textarea\" rows=\"4\" autocomplete=\"off\" style=\"width:10px\" ></textarea>" + "<input type=\"hidden\" class=\"combo-value\">" + "</span>";
        }
        var span = $(spanText).insertAfter(_847);
    ...
    function _856(_857) {
        ...
        if (!opts.disabled && !opts.readonly) {
            _85b.bind("click.combo", function (e) {
                ...
                //本来是keydown，现在改成keyup，因为keydown捕捉的keyCode不会是中文
                switch (e.keyCode) {
                    case 38:
    ...
    function _85f(_860) {
        ...
        function _865() {
            ...
            //
            var addComboClass = ".add-combobox-item,.add-combotree-item,.add-combogrid-item";
            if (top + _863._outerHeight() + $(addComboClass, _863.parent())._outerHeight() + $(".search-combobox-item", _863.parent())._outerHeight() > $(window)._outerHeight() + $(document).scrollTop()) {
                top = _862.offset().top - _863._outerHeight() - $(addComboClass, _863.parent())._outerHeight() - $(".search-combobox-item", _863.parent())._outerHeight();
            }
    ...
    function _85e(_866) {
        ...
        if (_867) {
            _867.panel && _867.panel.panel("close");
        }
    };
    function _868(_869) {
    ...
    function _872(_873) {
        ...
        var input = _875.find("input.combo-text");
        input = input.length == 0 ? _875.find("textarea.combo-text") : input;
        input.val("");
    };
    function _876(_877) {
        var _878 = $.data(_877, "combo").combo;
        var input = _878.find("input.combo-text");
        input = input.length == 0 ? _878.find("textarea.combo-text") : input;
        return input.val();
    };
    function _879(_87a, text) {
        var _87b = $.data(_87a, "combo");
        var _87c = _87b.combo.find("input.combo-text");
        _87c = _87c.length == 0 ? _87b.combo.find("textarea.combo-text") : _87c;
        if (_87c.val() != text) {
            _87c.val(text);
            $(_87a).combo("validate");
    ...
    $.fn.combo.methods = {
        ...
        textbox: function (jq) {
            return ($.data(jq[0], "combo").combo.find("input.combo-text").length == 0) ? $.data(jq[0], "combo").combo.find("textarea.combo-text") : $.data(jq[0], "combo").combo.find("input.combo-text");
        },
        destroy: function (jq) {
            return jq.each(function () {
                _850(this);
        ...
        resizeByItem: function (jq) {

            var itemSelector = ".combobox-item:visible,.combobox-group:visible,div.tree-node:visible";
            //先获取到panel
            var panel = $(jq).combo("panel");
            var opts = $(jq).combo("options");
            //没有找到的item样式
            var noMatchRecord = "no-match-records";
            //先隐藏没有找到的信息
            $("." + noMatchRecord, panel).hide();

            if ($(jq).combo("textbox").hasClass("datebox-input") || $(jq).hasClass("combogrid-f"))
                return;
            ...
            if (items.length == 0) {
                //是否已经有无记录匹配信息的显示了
                if ($("." + noMatchRecord, panel).length > 0) {
                    $("." + noMatchRecord, panel).show();
                }
                else {
                    //
                    panel.append($("<div class='" + noMatchRecord + "' style='display: block;'>&nbsp;&nbsp;" + (opts.hideNoMatch !== true ? HtmlLang.Write(LangModule.Common, "NoMatchRecords", "No Matched Records!") : "") + "</div>"));
                }
                //
                items = $("." + noMatchRecord, panel);
            }
            setComboPanelHeight(panel, items);
            setComboxPanelWidth(jq);
            //调整高度
            function setComboPanelHeight(panel, items) {
                if (items == undefined) {
                ...
                //加上滚动条高度
                if (items.length > 0 && panel.outerWidth() < panel.get(0).scrollWidth) {
                    itemHeight += 18;
                }
                //如果总高度高于元素的高度，则按照元素的高度来
                var panelHeight = panel.outerHeight();
                //
                panel.height(itemHeight > 198 ? 198 : itemHeight);
            }
            //调整宽度
            function setComboxPanelWidth(input, callback) {
                var panel = $(input).combo("panel");
                var options = $(input).data("combo").options;
                var scrollW = panel[0].scrollWidth;

                options.autoSizePanel = options.autoSizePanel == undefined ? true : options.autoSizePanel;

                //chenpan 隐藏横向滚动条
                panel.css("overflow-x", "hidden");
                if (options.autoSizePanel) {
                    var maxWidth = panel.width();

                    var itemList = panel.find(itemSelector);
                    ...
                    if (itemList && itemList.length > 0) {
                        ...
                        for (var i = 0; i < length; i++) {
                            ...
                            //是否有y方向的滚动条
                            var scrollWidth = 0;
                            if (tempItem.clientHeight < tempItem.offsetHeight) {
                                scrollWidth = 10;
                            }

                            maxWidth = $(tempItem).outerWidth(true) > maxWidth - scrollWidth ? $(tempItem).outerWidth(true) + scrollWidth : maxWidth;
                        }
                    }
                    ...
                    var different = 0;
                    //最顶级容器元素的宽度
                    var topDiv = $(input).closest(".m-imain");
                    if (topDiv && topDiv.length > 0) {
                        var topWidth = topDiv.width();
                        var inputLeft = $(panel).offset().left;
                        var different = topWidth - inputLeft - maxWidth;
                    }
                    //如果超过了顶级容器宽度
                    if (different < 0) {
                        maxWidth = topWidth - inputLeft - 10;
                    } else {
                        maxWidth = maxWidth > 500 ? 500 : maxWidth;
                    }
                    var defaultWidth = $(input).attr("defaultWidth");
                    if (defaultWidth) {
                        maxWidth = +defaultWidth;
                    }


                    panel.parent().width(maxWidth);
                    panel.width(maxWidth);

                } else if (scrollW >= panel.width()) {
                    panel.find(".combobox-item").width(scrollW);
                }
...
(function ($) {
    ...
    function nav(_8a4, dir) {
        ...
        if (item.length) {
            ...
            if (row) {
                ...
                //如果上下选择的时候有事件，则调用
                if (opts.navigation && $.isFunction(opts.navigation)) {
                    //
                    opts.navigation(_8a4, row[opts.valueField]);
                }
            }
        }
    };
    function _8a8(_8a9, _8aa) {
        ...
        _8ac(_8a9, _8ab);
        opts.onSelect.call(_8a9, opts.finder.getRow(_8a9, _8aa), _8a9);
    };
    ...
    function _8b6(_8b7, data, _8b8) {
        ...
        for (var i = 0; i < data.length; i++) {
            ...
            var row = data[i];
            //不显示在panel中的项 chenpan
            if (opts.hideItemKey) {
                if (row[opts.hideItemKey] == opts.hideItemValue) {
                    continue;
                }
            }
            //后台 有ComboboxMaxCount 控制，前端做这个多余了
            //var hidden = (opts.ComboboxMaxCount && opts.ComboboxMaxCount > 0 && addCount >= opts.ComboboxMaxCount);
            var v = row[opts.valueField] + "";
            var s = mText.encode(row[opts.textField]);
            var g = row[opts.groupField];
            //lgy
            if (s != undefined && vl != undefined && s != "" && vl != "" && s == $.trim(vl)) {
            ...
            //
            var formatterText = opts.formatter ? opts.formatter.call(_8b7, row) : "";
            //如果第一个是尖括号，就默认不进行编码了
            if (formatterText && formatterText.trim() && formatterText.indexOf("<") != 0) {
                //
                formatterText = mText.encode(formatterText);
            }
            //必须保证有格式化功能
            var comboboxItemInnerText = opts.formatter ? formatterText : s;

            //需要对title进行多一次的编码，这样的话，在html decode的时候只会进行一次的解码
            //dd.push("<div id=\"" + (_8b9.itemIdPrefix + "_" + i) + "\" class=\"" + cls + "\"  title='" + s + "'" + (hidden ? (" style='display:none;' ") : "") + ">");
            //后台 有ComboboxMaxCount 控制，前端做这个多余了
            dd.push("<div id=\"" + (_8b9.itemIdPrefix + "_" + i) + "\" class=\"" + cls + "\"  title='" + s + "'" + ">");


            dd.push(comboboxItemInnerText);
            dd.push("</div>");
        ...
        if ($(_8b7).combo("panel").is(":visible") && opts.mode !== "remote") {
            $(_8b7).combo("textbox").selectAll();
        }

        opts.onLoadSuccess.call(_8b7, data);

        $(_8b7).combo("resizeByItem");
    };
    function _8bc(_8bd, url, _8be, _8bf, _8zz) {
    ...
    function _8c0(_8c1, q) {
        ...
        if (opts.mode == "remote" && opts.url) {
            _8bc(_8c1, opts.url, { q: q }, true);
        } else {
            ...
            $.map(qq, function (q) {
                ...
                for (var i = 0; i < data.length; i++) {
                    ...
                    //remote表示从AJAX返回的数据，如果是从AJAX返回的，则最多展示20条，local不限制
                    //后台 有ComboboxMaxCount 控制，前端做这个多余了
                    //if (opts.mode == "remote") {
                    //    if (opts.ComboboxMaxCount && opts.ComboboxMaxCount > 0 && ff >= opts.ComboboxMaxCount) {
                    //        break;
                    //    }
                    //}

                }
                $(_8c1).combo("resizeByItem");
            });
            //如果没有找到一摸一样的，就拿第一个把 felson
            if (tt.length >= 1 && vv.length == 0) {
                vv.push(tt[0]);
            }
            _8ac(_8c1, vv, true);
        }
    ...
    function _8c5(_8c6) {
        ...
        //如果没有hover的，就去selected
        item = item.length ? item : _8c7.children("div.combobox-item-selected");
        if (item.length) {
        ...
        //
        if (t.attr("destroyed") != "1" && t.combobox("textbox") && t.combobox("textbox").is(":visible")) {
            var vv = [];
    ...
    function _8c9(_8ca) {
        ...
        opts, {
            onShowPanel: function () {
                ...
                $(_8ca).combo("resizeByItem");
            }
        }));
        $(_8ca).combo("textbox").parent().off("click.hidPanel").bind("click.hidPanel", function () {
            $(_8ca).combo("resizeByItem");
        })
        $(_8ca).combo("textbox").off("focus.hidPanel").bind("focus.hidPanel", function (e) {
            $(_8ca).combo("resizeByItem");
            ...
            var value = "";
            var data = _8cb.data;
            var oldValue = $(_8ca).combobox("getValue");
            for (var i = 0; i < data.length; i++) {
                var row = data[i];
                if (opts.hideItemKey) {
                    if (row[opts.hideItemKey] == opts.hideItemValue) {
                ...
                var s = row[opts.textField];
                if (s != "" && s.toLowerCase() == displayText) {
                    value = row[opts.valueField] + "";
                    break;
                }
            }
            if (oldValue != value) {
                opts.queryParams.v = value;
                opts.queryParams.q = "";
                $(_8ca).combobox("select", value);
                //if (opts.mode == "remote") {
                //    _8bc(this, $(_8ca), { v: value }, true, [value]);
                //}
            }
        ...
        $(_8ca).combo("panel").unbind().bind("mouseover", function (e) {
            ...
            if (item.length > 0 && !item.hasClass("combobox-item-disabled")) {
                item.addClass("combobox-item-hover");
                //如果上下选择的时候有事件，则调用
                if (opts.navigation && $.isFunction(opts.navigation)) {
                    //
                    opts.navigation(_8ca, opts.finder.getRow(_8ca, item)[opts.valueField]);
                }
            }
            e.stopPropagation();
        }).bind("mouseout", function (e) {
    ...
    $.fn.combobox.methods = {
        ...
        setValue: function (jq, _8d3) {
            return jq.each(function () {
                ...
                if (_8d3 != undefined && _8d3 != null && _8d3 != "" && opts.mode == "remote" && opts.queryParams.v != _8d3) {
                    var existsRow = false;
                    var row = opts.finder.getRow(this, _8d3);
                    if (row) {
                        _8ac(this, [_8d3]);
                    } else {
                        _8bc(this, null, { v: _8d3 }, true, [_8d3]);
                    }
                } else {
                    _8ac(this, [_8d3]);
                }
    ...
    $.fn.combo.defaults, {
        ...
        keyHandler: {
            ...
            enter: function (e) {
                _8c5(this);
                //因为 query 是delay 400 执行的。所以要等query 执行完成后再执行这个
                //凭证编辑页面有 destroy 逻辑，有冲突
                //setTimeout(function (aa) { _8c5(aa); }, 500, this);
            },
            query: function (q, e) {
                _8c0(this, q);
            }
        ...
        filter: function (q, row) {
            var opts = $(this).combobox("options");
            //
            var rowValue = row[opts.textField] || "";
            //如果加有m-account-combobox的样式，则filter不相同
            if ($(this).hasClass("m-account-combobox")) {
                //如果q是数字的话
                if ($.isNumeric(q) && q.toLowerCase().indexOf('%') != 0) {
                    //必须从头开始
                    return rowValue.toLowerCase().indexOf(q.toLowerCase()) == 0;
                }
                if (q.toLowerCase().indexOf('%') == 0) {
                    q = q.replace('%', '');
                }
			}
			return rowValue.toLowerCase().indexOf(q.toLowerCase()) > -1;
        },
...
(function ($) {
    function _8e9(_8ea) {
        ...
        //为文本框绑定点击事件，显示panel chenpan
        $(_8ea).parent().find(".combo-text").die("click").bind("click", function () {
            $(_8ea).combo("showPanel");
        });
        ...
        opts, {
            onShowPanel: function () {
                ...
                if (opts.hideItemKey == undefined) {
                    opts.hideItemKey = "MIsActive";
                }

                if (opts.hideItemValue == undefined) {
                    opts.hideItemValue = "0";
                }

                //combotree panel自适应宽度 chenpan
                var panel = $(this).combo("panel");
                //禁用横向滚动条
                panel.css("overflow-x", "hidden");

                //找到panel的所有tree node,然后算出一个最大长度
                var treeNodeDomList = panel.find(".tree-node");
                if (treeNodeDomList && treeNodeDomList.length > 0) {
                    var maxWidth = panel.width();
                    var length = treeNodeDomList.length;
                    for (var i = 0 ; i < length; i++) {
                        var treeNodeDom = treeNodeDomList[i];

                        var treeNode = tree.tree("getNode", treeNodeDom);

                        //如果是禁用的就不显示
                        if (treeNode) {
                            if (treeNode[opts.hideItemKey] == opts.hideItemValue) {
                                $(treeNodeDom).addClass("hidden-item").hide();
                                continue;
                            }
                        }
                        ...
                        $(treeNodeDom).find("span").each(function () {
                            tempWidth += $(this).width();

                            if ($(this).hasClass("tree-title")) {
                                var titleTips = $(this).text();

                                $(treeNodeDom).attr("title", titleTips);
                            }
                        });
                    ...
                    maxWidth = maxWidth > 500 ? 500 : maxWidth;
                    if (maxWidth > 0) {
                        panel.parent().css("width", maxWidth);
                        panel.css("width", maxWidth);
                    }
                }
            }
        }));
...
(function ($) {
    function _929(_92a) {
        ...
        $(_92a).combo("textbox").addClass("datebox-input");
