...
; (function () {
    ...
    $.extend($.XYTipsWindow, {
        ...
        init: function (o) {
            ...
            o.zIndexScale = o.mContent.substring(0, o.mContent.indexOf(":")) == 'id' ? 10 : 1;
            o.divFrame = o.mContent.substring(0, o.mContent.indexOf(":")) == 'id';
            ...
            }).css({ zIndex: Math.round(870618 / o.zIndexScale).toString() });
            //最大化功能
            ...
            if (o.mShowbg != "" && o.mShowbg == true) {
                var $boxBgDom = "<div id=\"XYTipsWindowBg\" boxid=\"" + o.mBoxID + "\" style=\"position:absolute;overflow:hidden;height:" + $(document).height() + "px;background:" + o.mWindowBgColor + ";filter:alpha(opacity=0);opacity:0;width:100%;left:0;top:0;z-index:" + Math.round(870618 / o.zIndexScale) + "\"><iframe src=\"about:blank\" style=\"width=100%;height:" + ($(document).height()) + "px;filter:alpha(opacity=0);opacity:0;scrolling=no;z-index:" + Math.round(870610 / o.zIndexScale) + "\"></iframe></div>";
            ...
            //点击黑色遮罩，不需要做任何处理
            $(document).off("click.top", "#XYTipsWindowBg").on("click.top", "#XYTipsWindowBg",
                function (event) {
                    //
            ...
            //
            o.mOpenCallback && $.isFunction(o.mOpenCallback) && o.mOpenCallback();
        },
        getID: function () {
        ...
        closeBox: function (o) {
            ...
            //获取参数
            var index = $tipsWindow.attr("index") || 0, param = $tipsWindow.attr("param" + "_" + index);
            //判断param是否是JSON格式的，如果是的话，就parse一下
            if (param && param.length > 0 && param.startWith("{") && param.endWith("}")) {
                //
        ...
        callback: function (o) {
            o = o || $.XYTipsWindow.options;
            o.mCallback && $.isFunction(o.mCallback) && o.mCallback();
        },
        //如果本身就在弹出框中
        handlePanrentTile: function (max, currentID) {

            var tipsWindow = $(".XYTipsWindow", window.document);

            if (tipsWindow.length > 1) {
                tipsWindow = $(".XYTipsWindow:not([id='" + currentID + "'])", window.document);
            }

            if (tipsWindow.length > 0 && tipsWindow.attr("divframe") == "1" && tipsWindow.attr("id") != currentID) {
                if (max === true && $(".mMaxBox", tipsWindow).is(":visible")) {
                    window.$.XYTipsWindow.maxBox();

                    return $(".mCloseBox", window.document).attr("boxid");
                }
                else if (max === false && $(".mMinBox", $(".XYTipsWindow", window.document)).is(":visible")) {


                    window.$.XYTipsWindow.minBox(tipsWindow.attr("parentid"));

                    tipsWindow.parent().siblings(".XYTipsWindowBg").css({ "z-index": "70618" });
                }
                return;
            }
            //
        ...
        showBox: function (o) {
            //处理
            var parentID = $.XYTipsWindow.handlePanrentTile(true);


            var dialogH = o.mHeight;
            //如果有设置弹框高度
            if (dialogH != undefined && window != top) {
                var containerH = $(".m-main", parent.document).height() || $(".mBoxContent", parent.document).height();
                var maxH = containerH - 55;
                if (maxH > 0) {
                    //弹框高度不能超出容器高度
                    o.mHeight = Math.min(maxH, o.mHeight);
                }
            }

            //是否显示标题栏，决定标题栏的高度
            var $titleHeight = o.mShowTitle ? 50 : 0,
				$borderHeight = 0;
            ...
            //第二层
            var $boxDom = "<div id=\"" + o.mBoxID + "\" class=\"XYTipsWindow\" divframe= \"" + (o.divFrame ? "1" : "0") + "\" parentid=\"" + parentID + "\">";
            //第三层
            $boxDom += "<div class=\"mBoxWrap\">";
            $boxDom += "<div class=\"mBoxTitle\"><h3></h3><span class=\"mMaxBox\">最大化</span><span class=\"mMinBox\"  style=\"display:none;\">最小化</span><span class=\"mCloseBox\"  boxID=\"" + o.mBoxID + "\" parentid=\"" + parentID + "\">关闭</span></div>";
            //第四层
            ...
            //第五层
            $boxDom += "<iframe src=\"about:blank\" style=\"position:absolute;left:0;top:0;filter:alpha(opacity=0);opacity:0;scrolling=no;z-index:" + Math.round(10714 / o.zIndexScale) + "\"></iframe>";
            $boxDom += "</div>";
            $($boxDom).appendTo("body");
            //是否启用最大化最小化
            if (!o.mMax || (winHeight == o.mHeight + $titleHeight && winWidth == o.mWidth)) {
            ...
            $box.css({
                ...
                zIndex: Math.round(891208 / o.zIndexScale).toString()
            });
            ...
            $boxWrap.css({
                ...
                overflow: "hidden",
                zIndex: Math.round(20590 / o.zIndexScale).toString(),
                //"box-shadow": o.mShowTitle ? "0" : "0 0 10px 10px #ccc",
                border: o.mShowTitle ? "0px" : "border: 1px solid #ccc;"
            });
            ...
            $boxBg.css({
                ...
                zIndex: Math.round(10715 / o.zIndexScale).toString()
            });
            ...
            if (o.mOffsets == "" || o.mOffsets.constructor == String) {
                switch ($location) {
                    ...
                    case ("right-middle")://右边居中
                        ...
                        break;
                    default://默认为居中
                        $location = { left: "50%", marginLeft: -parseInt($box.width() / 2) + "px", top: "50%" + est, marginTop: -parseInt(($box.height() + 4) / 2) + "px" + est };
                        TOP = $getPageSize[1] / 2 - $box.height() / 2;
                        break;
                };
            } else {
        ...
        contentBox: function (o) {
            ...
            $content = $content && ($contentType == "url" || $contentType == "iframe") ? Megi.addTabTitle($content, o.mTitle) : $content;
        ...
        setBoxzIndex: function (o) {
            ...
            $box.die().live("click", function (event) {
                for (var i = 0; i < $XYTipsWindowarr.length; i++) {
                    $XYTipsWindowarr[i].style.zIndex = Math.round(870618 / o.zIndexScale);
                };
                this.style.zIndex = Math.round(891208 / o.zIndexScale);
        ...
        dragBox: function (o) {
            ...
            $Handle.mousedown(function (e) {
                ...
                $(document).off("mousemove.xy").on("mousemove.xy", function (e) {
                    if (drag) {
                        e = window.event ? window.event : e;
                        window.getSelection ? window.getSelection().removeAllRanges() : document.selection.empty();
                        var ___x = e.clientX - $moveX;
                        var ___y = e.clientY - $moveY;
                        $("#XYTipsWindowBg").css({ "background-color": "#001" })
                        $.XYTipsWindow.setBoxNewPosition($(___ID), ___x, ___y);
                        $("#XYTipsWindowBg").css({ "background-color": "#000" })
                    };
                });
        ...
        //设置窗口新位置
        setBoxNewPosition: function ($box, left, top) {
            var boxTitle = $(".mBoxTitle", $box);
            var boxParent = $box.parent();
            var boxParentPos = boxParent.offset();
            var parentIfr = $.mTab.getCurrentIframe().contents();
            var parentIfrH = parentIfr.height();
            var parentIfrW = parentIfr.width();
            var minTop = -boxParentPos.top;
            var maxTop = parentIfrH - boxParentPos.top - boxTitle.height();
            var minLeft = -boxParentPos.left;
            var maxLeft = parentIfrW - boxParentPos.left - $box.width();
            if (top < minTop) {
                top = minTop;
            }
            else if (top > maxTop) {
        ...
        removeBox: function () {
            //处理
            $.XYTipsWindow.handlePanrentTile(false, BOXID.mBoxID);
            var $box = $("#" + BOXID.mBoxID);
            var $boxbg = $("div[id='XYTipsWindowBg'][boxid='" + BOXID.mBoxID + "']");
        ...
        maxBox: function () {
            ...
            if ($xyTipWindow.length == 0) {

                $xyTipWindow = $("#" + $(".mCloseBox").attr("boxID"));
            }
            //第一层
            var $box = $xyTipWindow.parent();
            //
            if ($box.width() == winWidth && $box.height() == winHeight) {
                //如果本身就是最大化了，就不要再最大化了
                return true;
            }
            //第三层
            ...
            $boxBd.css(
                {
                    height: (winHeight - $boxTitle.height()) + "px",
                    width: winWidth + "px"
                });
            //调节$boxTitle
            $boxTitle.css(
            ...
            //调用计算高度的函数
            $iframe.length > 0 && $($iframe[0].contentWindow).resize();
        },
        //还原弹出层
        minBox: function (parentID) {
            //第二层
            var $xyTipWindow = $("#" + (parentID || BOXID.mBoxID));
        ...
        keyDown: function (o) {
            ...
            $(document).off("keydown.xytip").on("keydown.xytip", function (e) {
                e = e || event;
                if (e.keyCode == 27) {
                    $.XYTipsWindow.removeBox();
                };
