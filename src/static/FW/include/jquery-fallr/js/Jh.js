...
Jh.Util = {//工具类
	...
	toBody:function(o){//往Body添加对象
	    $(Jh.Config.containerSelector).append(o);
	}
};
...
Jh.Portal = function(me){//Portal对象
	var _box = "<div id='portal'></div>",
		_template = {//模板
			l :"<div id='"+Jh.Layout.location.left+"' class='"+Jh.Config._groupWrapperClass+" wl50'/>",
			m :"<div id='"+Jh.Layout.location.center+"' class='"+Jh.Config._groupWrapperClass+" wr50'/>",
			r: "<div id='" + Jh.Layout.location.right + "' class='" + Jh.Config._groupWrapperClass + " wnone'/>",
	...
	return me={	
		...
		_createPortalOne:function(key,name){//创建单个portal item
			var itemHeader  =  me._createItemHeader(name),//header
			    itemContent =  me._createItemContent(name),//content
		...
		_createItemContent:function(name){//创建content
		    var _content = $(_template.itemContent);

		    var contentHtml = $("#mg" + name).html();
		    $("<div>" + contentHtml + "</div>").appendTo(_content);
		    $("#mg" + name).remove();
			return _content;
		},
