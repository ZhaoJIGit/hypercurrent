(function ($, window, document, undefined) {
	...
	function showStep(skipDirection) {
		var options = _activeWalkthrough;
		var step = options.steps[_index];
		var targetElement = options._element.find(step.wrapper);
		if (step.iframe != "") {
			var document = $(step.iframe).contents();
			targetElement = document.find(step.wrapper);
		}
	...
	function buildWalkthrough() {
		...
		if (step.popup.type !== "modal" && step.popup.type !== "nohighlight") {
			...
			$("<div>").addClass("overlay-hole").height(Math.min(maxHeight, targetElement.outerHeight())).width(targetElement.outerWidth()).css({
				padding: "20px",
				position: "absolute",
				top: targetElement.offset().top - 20 + iframeTop,
				left: targetElement.offset().left - 20 + iframeLeft,
				"z-index": 999998,
	...
	function showTooltip() {
		...
		$jpwTooltip.html("").css({
			...
			id: "tooltipWrapper",
			style: "width:" + cleanValue(parseInt(step.popup.width, 10))
		})).appendTo($jpWalkthrough);
		$jpWalkthrough.appendTo("body").show();
		$("#tooltipWrapper").css(textRotation);
		$("#tooltipInner").append(getContent(step)).show();
	...
	function showButton(id, appendTo) {
		...
		if (id == "jpwNext" && isLastStep())
			return;
		if (id == "jpwPrevious" && isFirstStep())
			return;
		if (id == "jpwFinish" && !isLastStep())
			return;
		$(appendTo).append($("<a />", {
			id: id,
			html: btn.i18n
	...
	$.pagewalkthrough = $.fn.pagewalkthrough = function (method) {
		...
		if (method == "next" && isFirstStep()) {
			//
			$("#aOption").trigger("click");
		}
