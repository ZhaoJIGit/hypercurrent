(function ($, window, document, undefined) {
    ...
    var methods = {
        ...
        close: function () {
            ...
            _isCookieLoad = getCookie("_walkthrough-" + _activeId);
            $jpwOverlay.fadeOut("fast",
            function () {
                $(this).remove()
            });
            $jpWalkthrough.fadeOut("fast",
            function () {
                $(this).html("").remove()
            });
            $("#jpwClose").fadeOut("fast",
            function () {
                $(this).remove()
            })
        },
    ...
    function buildWalkthrough() {
        ...
        showButton("jqwSkip", "#bottom-button");
        showButton("jpwPrevious", "#bottom-button");
        showButton("jpwNext", "#bottom-button");
        showButton("jpwFinish", "#bottom-button")
    }
    function showModal() {
        var options = _activeWalkthrough,
        step = options.steps[_index];
        ...
        $jpwTooltip.css({
            ...
            left: "50%",
            top: "25%",
            "margin-left": -(parseInt(step.popup.width, 10)) / 2 + "px",
            "z-index": "999999",
            width: parseInt(step.popup.width, 10),
            height: parseInt(step.popup.height, 10),
            "border-radius": "5px"
        });
        ...
        })).append('<div id="bottom-button"></div>').appendTo($jpWalkthrough);
        $jpWalkthrough.appendTo("body");
        $jpwTooltip.show();

        if (step.popup.titleimageclass) {
            $("#tooltipInner").append("<div class='" + step.popup.titleimageclass + "'>&nbsp;</div>");
        }
    ...
    function showTooltip() {
        ...
        $("#tooltipWrapper").css(textRotation);
        $("#tooltipInner").append(getContent(step)).show();
        if (step.popup.textalign) {
            $("#tooltipInner").css({
                "text-align": step.popup.textalign
            });
        }
    ...
    function getContent(step) {
        ...
        content;
        try {
            content = step.popup.titletext;

            var detailTextArray = step.popup.detailtext;

            if (detailTextArray && detailTextArray.length > 0) {
                for (var i = 0 ; i < detailTextArray.length; i++) {
                    var detailtext = detailTextArray[i];

                    content += "<br><span class='page-wizard-detail'>" + detailtext + "</span>";
                }
            }
    ...
    $(document).on("click", "#jpwClose, #jpwFinish,#jqwSkip", methods.close);
    ...
    $.fn.pagewalkthrough.defaults = {
        ...
        buttons: {
            jpwClose: {
                i18n: HtmlLang.Write(LangModule.Common, "PageWizardClose", "点击这里关闭向导"),
                show: true
            },
            jqwSkip: {
                i18n: HtmlLang.Write(LangModule.Common, "PageWizardSkip", "不用了"),
                show: function () {
                    return isFirstStep()
                },
                classname: "easyui-linkbutton easyui-linkbutton-gray l-btn"
            },
            jpwNext: {
                i18n: HtmlLang.Write(LangModule.Common, "PageWizardNext", "下一步"),
                show: function () {
                    return !isLastStep()
                },
                classname: "easyui-linkbutton easyui-linkbutton-yellow l-btn"
            },
            jpwPrevious: {
                i18n: HtmlLang.Write(LangModule.Common, "PageWizardPrevious", "上一步"),
                show: function () {
                    return !isFirstStep()
                },
                classname: "easyui-linkbutton easyui-linkbutton-gray l-btn "
            ...
            jpwFinish: {
                i18n: HtmlLang.Write(LangModule.Common, "PageWizardFinish", "开始使用"),
                show: function () {
                    return isLastStep()
                },
                classname: "easyui-linkbutton easyui-linkbutton-yellow l-btn"
