var AccountBalances = {
    ...
    initAction: function () {
        ...
        $("#btnFinish").click(function () {
            ...
            if (AccountBalances.accountStandard == 3) {
                AccountBalances.checkCustomAccountIsMatch(function () {
                    AccountBalances.checkBalanceEqualWithBill(func);
                })
            } else {
                AccountBalances.checkBalanceEqualWithBill(func);
            }
        ...
        mAjax.post(
            ...
            function (msg) {
                if (msg) {
                    //保存为全局变量
                    if (msg) {
                        AccountBalances.bankList = msg.Data;
                    }
                }
            });
    },
    ...
    bindGrid: function () {
        ...
        $("#tbAccountBalance").treegrid({
            ...
            onClickCell: function (field, row) {
                ...
                $.mTab.addOrUpdate(title, AccountBalances.go + '/BD/BDAccountBalance/BalanceInput?defaultId=' + row.id, true, true, false, true);
            }
        });
    },
    //更新一级科目行的背景色
    ...
    createBalanceTable: function (list, field, canEdit, baseCurrencyField) {
        ...
        if (list && list.length > 0) {
            ...
            for (var i = 0 ; i < list.length; i++) {
                ...
                html += "<tr>";
                html += "<td style='border:0px;text-align:left;  width:10%'><div class='id-for-li-currency'>" + currency + "</div></td><td  style='border:0px;text-align:right;color:#7EB8EA'>" + Megi.Math.toMoneyFormat(money) + "</td>";
                html += "</tr>";
            ...
            if (currencyType > 1 || oldCurrency != AccountBalances.baseCurrency) {
                html += "<tr><td style='border:0px; text-align:left; width:10%;font-size:13px'>" + AccountBalances.baseCurrency + "(" + HtmlLang.Write(LangModule.GL, "Total", "合计") + ")</td><td style='border:0px;text-align:right;font-size:13px'>" + Megi.Math.toMoneyFormat(count) + "</td></tr>";
            }
