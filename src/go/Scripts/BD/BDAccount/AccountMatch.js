...
var AccountMatch = {
    ...
    bindGrid: function () {
        ...
        $(AccountMatch.selector).treegrid({
            ...
            columns: [[
                ...
                },
                {
                    ...
                    formatter: function (value, row, index) {
                        if (row.MNumber) {
                            return "<input type='text' class='unmatch-sys-account-cb easyui-combobox' data-options='required:true' pnumber='" + row._parentId + "' number='" + row.MNumber + "' initval='" + row.MMatchNumber + "' style='width:230px;height:25px;border:1px solid #c2cbd3;'/>";
                        }
                        return "";
                    }
                },
            ...
            onLoadSuccess: function (row, data) {
                ...
                AccountMatch.unMatchSystemAccountList = data[0].SystemAccountList.filter(function (el) {
                    return $.inArray(el.MNumber, arrMatchedUserNumber) == -1;
                });
                var cbUnMatch = $(AccountMatch.cbUnMatchSysAcctClass);
    ...
    bindCombobox: function (sender, options) {
        var opt = {
            valueField: 'MNumber',
            textField: 'MFullName',
            height: 30
