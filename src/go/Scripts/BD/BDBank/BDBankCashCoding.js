var BDBankCashCoding = {
    ...
    IsCanSettingChangePermission: false,    //是否有 基础资料 编辑权限
    AccountDataSource: [],
    BankID: "",
    //查找相关
    functoolbar: ".cash-coding",
    btnSearchdiv: ".btnSearchDiv",
    btnreset: '.search-reset',
    btnSearchclose: ".m-adv-search-close",
    divSearch: ".m-adv-search-cashcoding",
    ...
    //是否正在重新获取联系人
    isReloadingContact: false,
    InitCashCodingList: function (bankID) {
        BDBankCashCoding.initDom();
        ...
        BDBankCashCoding.IsCanSettingChangePermission = $("#hidIsCanSettingChangePermission").val() == "1" ? true : false;
        ...
        //显示搜索div
        $(BDBankCashCoding.btnSearchdiv, BDBankCashCoding.functoolbar).off("click").on("click", function () {
            $(BDBankCashCoding.btnSearchdiv, BDBankCashCoding.functoolbar).hide();
            $(BDBankCashCoding.divSearch).show();
            $("#gridCashCoding").datagrid("resize", { height: BDBankCashCoding.getGridHeight() });
        });

        //隐藏搜索div
        $(BDBankCashCoding.btnSearchclose, BDBankCashCoding.divSearch).off("click").on("click", function () {
            $(BDBankCashCoding.btnSearchdiv, BDBankCashCoding.functoolbar).show();
            $(BDBankCashCoding.divSearch).hide();
            $("#gridCashCoding").datagrid("resize", { height: BDBankCashCoding.getGridHeight() });
        });

        $(BDBankCashCoding.exactAmount, BDBankCashCoding.divSearch).off("change").on("change", BDBankCashCoding.exactAmountShow);

        $(BDBankCashCoding.btnreset, BDBankCashCoding.divSearch).off("click").on("click", function () {
            $(BDBankCashCoding.txtDate, BDBankCashCoding.divSearch).datebox('setValue', '');
            $(BDBankCashCoding.txtPayer, BDBankCashCoding.divSearch).val('');
            $(BDBankCashCoding.txtRef, BDBankCashCoding.divSearch).val('');
            $(BDBankCashCoding.txtFrom, BDBankCashCoding.divSearch).combobox("setValue", '');
            $(BDBankCashCoding.txtAmountFrom, BDBankCashCoding.divSearch).numberbox("setValue", "");
            $(BDBankCashCoding.txtAmountTo, BDBankCashCoding.divSearch).numberbox("setValue", "");
            $(BDBankCashCoding.exactAmount, BDBankCashCoding.divSearch).prop("checked", false);
            $(BDBankCashCoding.exactAmount, BDBankCashCoding.divSearch).trigger("change");
        });
        ...
        $(BDBankCashCoding.btnSearch, BDBankCashCoding.divSearch).off("click").click(function () {
    ...
    //初始化界面元素
    initDom: function () {
        $(BDBankCashCoding.txtPayer, BDBankCashCoding.divSearch).removeAttr("hint").attr("hint", HtmlLang.Write(LangModule.Bank, "Payee", "收/付款人")).initHint();
        //支出\收入
        $(BDBankCashCoding.txtFrom, BDBankCashCoding.divSearch).combobox({
            width: 110,
            textField: 'text',
            valueField: 'value',
            data: [
    ...
    exactAmountShow: function () {
        if ($(BDBankCashCoding.exactAmount, BDBankCashCoding.divSearch).is(":checked")) {
            $(BDBankCashCoding.txtAmountTo, BDBankCashCoding.divSearch).hide();
            $(BDBankCashCoding.toFlag, BDBankCashCoding.divSearch).hide();
            $(BDBankCashCoding.txtAmountFrom, BDBankCashCoding.divSearch).removeAttr("hint").attr("hint", HtmlLang.Write(LangModule.Common, "Amount", "Amount")).removeClass("has-hint").initHint();
        }
        else {
            $(BDBankCashCoding.txtAmountTo, BDBankCashCoding.divSearch).show();
            $(BDBankCashCoding.toFlag, BDBankCashCoding.divSearch).show();
            $(BDBankCashCoding.txtAmountFrom, BDBankCashCoding.divSearch).removeAttr("hint").attr("hint", HtmlLang.Write(LangModule.Common, "MinAmount", "最小金额")).removeClass("has-hint").initHint();
        }
    },
    getFiter: function () {
        var filter = {
            ExactDate: $(BDBankCashCoding.txtDate, BDBankCashCoding.divSearch).datebox("getValue"),
            TransAcctName: $(BDBankCashCoding.txtPayer, BDBankCashCoding.divSearch).val(),
            MDesc: $(BDBankCashCoding.txtRef, BDBankCashCoding.divSearch).val(),
            SrcFrom: $(BDBankCashCoding.txtFrom, BDBankCashCoding.divSearch).combobox("getValue"),
            AmountFrom: $(BDBankCashCoding.txtAmountFrom, BDBankCashCoding.divSearch).val(),
            AmountTo: $(BDBankCashCoding.txtAmountTo, BDBankCashCoding.divSearch).val(),
            IsExactAmount: $(BDBankCashCoding.exactAmount, BDBankCashCoding.divSearch).is(":checked"),
        };
        return filter;
    },
    ...
    reloadBalance: function () {
        ...
        mAjax.Post("/BD/BDBank/GetBDBankAccountInfoByDate",
            { acctid: id, beginDate: dates[0], endDate: dates[1] },
             function (data) {
                 $("span[name='MBankStatement']").html(data.MBankStatement);
                 $("span[name='MMegiBalance']").html(data.MMegiBalance);
             });
    },
    getCheckedIds: function () {
        //获取所有的checkbox
        var checkboxs = $("input.row-key-checkbox[type='checkbox']:checked", $("#gridCashCoding").closest(".datagrid"));
        var checkedIds = [];
     ...
     */
    getCodingModels: function () {
        //所有选中的项
        var selectedCodings = [];
        var checkedIds = BDBankCashCoding.getCheckedIds();
        var rows = Megi.grid("#gridCashCoding", "getRows");
        for (var i = 0; i < rows.length && checkedIds.length > 0; i++) {
            if (checkedIds.contains(rows[i].MEntryID.toString())) selectedCodings.push(rows[i]);
        }
    ...
    saveAndRec: function () {
        ...
        var rows = Megi.grid("#gridCashCoding", "getRows");
        //  var selectRecs = Megi.grid("#gridCashCoding", "getSelections");
        var selectRecs = BDBankCashCoding.getCodingModels();
        ...
        mAjax.submit(
            ...
            function (msg) {
                if (msg.Success) {
                    ...
                    $.mDialog.alert("<div class='popup-list-msg'>" + msg.Message + "</div>", function () {
                        BDBankCashCoding.bindGrid(BDBankCashCoding.BankID);
                    }, 1, true);
                }
            });
    ...
    bindGrid: function (bankID) {
        ...
        BDBankCashCoding.isReloadingContact = false;
        var columns = BDBankCashCoding.getColumns();
        var filter = BDBankCashCoding.getFiter();
        ...
        Megi.grid("#gridCashCoding", {
            ...
            onLoadSuccess: function () {
                ...
                //bug 24582 要求切换页后，从改页的首条开始显示 
                $("#gridCashCoding").datagrid("scrollTo", 0);
    ...
    reloadTaxRateData: function (taxRateModel) {
        var taxRateId = taxRateModel.MItemID;
        var rowInfo = BDBankCashCoding.getEditingRowInfo();
        if (rowInfo.RowIndex == -1) {
            return;
        }
        BDBankCashCoding.beginEditRow(rowInfo.RowIndex);
        var editor = $("#gridCashCoding").datagrid('getEditor', { index: rowInfo.RowIndex, field: "MTaxID" });
        if (editor == null) {
            return;
        }
        var target = editor.target;
        $(target).combobox("textbox").click().focus();
        $(target).combobox("select", taxRateId);
    },
    ...
    getEditorAmt: function (value) {
        ...
        value = mMath.toDecimal(value, 2); //Number(value);
    ...
    updateRootSpentRowData: function (rootId, entryId, spentAmt, inputObj) {
        ...
        entryTotalSpentAmt = mMath.toDecimal(entryTotalSpentAmt, 2);
        var money = (Number(obj.MSrcSpentAmt)).sub(entryTotalSpentAmt).sub(spentAmt);
        if (money <= 0) {
    ...
    updateRootReceiveRowData: function (rootId, entryId, receiveAmt, inputObj) {
        ...
        entryTotalReceiveAmt = mMath.toDecimal(entryTotalReceiveAmt, 2);
        var money = (Number(obj.MSrcReceivedAmt)).sub(entryTotalReceiveAmt).sub(receiveAmt);
        if (money <= 0) {
            if (inputObj != undefined) {
                $(inputObj).val("");
            }
    ...
    //更新model的属性值
    UpdateModelFiledValue: function (rowData, field, value) {
        rowData[field] = value;
    },
    ...
    getColumns: function () {
            ...
            title: HtmlLang.Write(LangModule.Common, "FastCode", "Fast Code"), field: 'MFastCodeID', width: 100, align: 'left', formatter: function (value, rec, rowIndex) {
                ...
                options: {
                    addOptions: {
                        ...
                        hasPermission: BDBankCashCoding.IsCanSettingChangePermission,
                    ...
                    dataOptions: {
                        ...
                        onSelect: function (selector, record) {
                            if (record != undefined) {
                                ...
                                ///add by 锦友 2018-09-04
                               ///因为联系人selected会默认带出往来科目给科目赋值，所以给科目赋值要在联系人selected之后
                                BDBankCashCoding.setEditorValue("combobox", rowInfo.RowIndex, "MAccountID", record.MAccountID);
                                if (BDBankCashCoding.IsGL == "1") {
                ...
                options: {
                    ...
                    dataOptions: {
                        ...
                        onSelect: function (record) {
                            var recordMItemID = "";
                            if (record != undefined) {
                                recordMItemID = record.MItemID;
                                //更新行数据的联系人名称字段
                                BDBankCashCoding.updateRowData("MContactName", record.MName);
                            }
                            BDBankCashCoding.bindGridComboxEditorEvent("MContactID", recordMItemID);
                            BDBankCashCoding.setDefaultAccount(recordMItemID);
                        }
                    }
                }
            }, formatter: function (value, rec, rowIndex) {
                //优先取自动匹配的联系人名称（手动修改已匹配联系人不会刷新页面，也是从rec.MContactName取）、手动保存的联系人名称从数据源获取 -- by linfq
                return rec.MContactName || BDBankCashCoding.getContactName(value);
            }
        });
        ...
        arr.push({
            title: HtmlLang.Write(LangModule.Bank, "TaxRate", "Tax Rate"), field: 'MTaxID', width: 100, align: 'left',
            editor: {
                type: 'addCombobox',
                options: {
                    type: "taxrate",
                    ...
                    addOptions: {
                        ...
                        //是否有基础资料编辑权限
                        hasPermission: BDBankCashCoding.IsCanSettingChangePermission,
                        callback: function (taxrateModel) {
                            BDBankCashCoding.TaxRateDataSource = mData.getTaxRateList();
                            BDBankCashCoding.reloadTaxRateData(taxrateModel);
                        }
                    },
                    dataOptions: {
                        height: "24px",
                        ...
                        onSelect: function (record) {
                            var taxrateMItemID = "";
                            if (record != undefined) {
                                taxrateMItemID = record.MItemID;
                            }
                            BDBankCashCoding.bindGridComboxEditorEvent("MTaxID", taxrateMItemID);
                        }
                    ...
                    options: {
                        ...
                        dataOptions: {
                            ...
                            onSelect: function (record) {
                                ...
                                BDBankCashCoding.bindGridComboxEditorEvent("MAccountID", recordMItemID);
                            }
                        }
                    }
    ...
    //更新行数据字段值
    updateRowData: function (fieldName, fieldValue) {
        var rowInfo = BDBankCashCoding.getEditingRowInfo();
        var grid = $('#gridCashCoding');
        var data = $.data(grid[0], 'datagrid').data;
        var rows = data.rows;
        rows[rowInfo.RowIndex][fieldName] = fieldValue;
    },
    //重新选择Combobox
    ...
    getContactName: function (itemId) {
        if (BDBankCashCoding.ContactDataSource == null || BDBankCashCoding.ContactDataSource.length == 0 || !itemId) {
            return "";
        }
        
        var contactName = BDBankCashCoding.getContactNameFromDataSource(itemId);
        //如果找不到联系人，则需要重新获取联系人并更新数据源
        if (!contactName && !BDBankCashCoding.isReloadingContact) {
            BDBankCashCoding.isReloadingContact = true;
            BDBankCashCoding.getContactData(function () {
                BDBankCashCoding.isReloadingContact = false;
            });
        }
    ...
    //从数据源查找联系人
    getContactNameFromDataSource: function(itemId){
    ...
    getTrackColumn: function (index) {
        ...
        obj.width = 80;
        obj.editor = BDBankCashCoding.getTrackItemEditor(index, obj);
        obj.formatter = function (value, rowData, rowIndex) {
            return BDBankCashCoding.getTrackName(value);
        }
        return obj;
    },
    getTrackItemEditor: function (index, obj) {
        var thisTrackDataSourceData = BDBankCashCoding.TrackDataSource[index];
        var parentId = thisTrackDataSourceData.MValue
        var tracks = thisTrackDataSourceData.MChildren;
        var editor = {
            type: "addCombobox",
            options: {
                type: 'trackOption',
                dataOptions: {
                    ...
                    onSelect: function (record) {
                        ...
                        BDBankCashCoding.bindGridComboxEditorEvent(obj.field, mvalue);
                    },
                    onLoadSuccess: function (data, textbox) {
                    }
                },
                addOptions: {
                    hasPermission: BDBankCashCoding.IsCanSettingChangePermission,
                    ...
                    callback: function (model) {
                        if (!!model && !!model.id) {
                            var thisModel = tracks[0];
                            thisModel.MName = model.text;
                            thisModel.MValue = model.id;
                            tracks.push(thisModel);
                            var rowInfo = BDBankCashCoding.getEditingRowInfo();
                            var editor = $("#gridCashCoding").datagrid('getEditor', { index: rowInfo.RowIndex, field: obj.field });
                            if (editor == null) {
                                return;
                            }
                            var target = editor.target;
                            $(target).combobox("loadData", tracks);
                            $(target).combobox("select", model.id);
                            $(target).combobox("hidePanel");
                            BDBankCashCoding.fillTrackItemDta(obj.field, tracks);
                        }
                    }
                }
            }
    ...
    //添加跟踪项时，给跟踪项下拉列表添加新添加的值
    fillTrackItemDta: function (field, data) {
        $("#gridCashCoding").datagrid("options").columns[0].where("x.field == '" + field + "'")[0].editor.options.dataOptions.data = data;
    },
