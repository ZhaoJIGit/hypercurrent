...
(function () {
    ...
    var FCHome = (function () {
        ...
        var FCHome = function () {
            ...
            //定义各个tab对应的index
            var voucher = 0, cashCoding = 1, invoice = 2;
            //
            ...
            this.showTab = function (index, subIndex) {
                ...
                switch (index) {
                    ...
                    case invoice:
                        //如果没有初始化，则初始化
                        !inited && new FCInvoiceModuleList().init(subIndex);
                        //加上初始化标志
                        currentPartial.attr("inited", 1);
                        //
                        break;
            ...
            this.updateModuleInfo = function () {
                ...
                mAjax.post(getModuleInfoUrl, {}, function (data) {
                    ...
                    //}
                    if ($("#isCanEditVoucherPermission").val()) {
                        that.initTabTitle(0, data[0] > 1 ? (data[0] + " " + items) : (data[0] + " " + item));
                        that.initTabTitle(1, data[1] > 1 ? (data[1] + " " + items) : (data[1] + " " + item));
                    } else {
                        that.initTabTitle(0, data[1] > 1 ? (data[1] + " " + items) : (data[1] + " " + item));
                    }
                });
            };
            //获取科目数据
            this.getAccountData = function (func) {
                //异步获取
                mAjax.post(getAccountUrl, { isIncludeParent: false, needFullName: true }, function (data) {
                    //
            ...
            this.initTabTitle = function (index, title) {
                //
                $(".fc-main-tabs li:eq(" + index + ") .title").text(title);
            };
            //初始化
            ...
            //保存凭证模板
            this.saveVoucherModule = function (voucher, transfer, callback1, callback2) {
                //
                var url = transfer ? updateVoucherByPeriodTransfer : updateVoucherUrl;
                //
                var params = transfer ? { voucher: voucher, transfer: transfer } : voucher;
                //
                voucher && mAjax.submit(url, params, callback1, callback2, true);
            }
