var FCCashCodingEdit = {
            ...
            for (var i = 0; i < trackCount; i++) {
                ...
                for (var j = 0; j < mChildrens.length; j++) {
                    ...
                    if (mChildrens[j].MValue1 === "1") {
                        mChildrens[j].MIsActive = true;
                    } else {
                        mChildrens[j].MIsActive = false;
                    }
    ...
    saveCashCodingEdit: function () {
        $("body").mFormSubmit({
            url: "/FC/FCCashCodingModule/UpdateCashCodingModule", param: { model: {} }, callback: function (msg) {
                if (msg.Success) {
                    $.mMsg(HtmlLang.Write(LangModule.Common, "SaveSuccessful", "Save Successful！"));
                    $.mDialog.close(0, msg.ObjectID);

                } else {
                    $.mDialog.alert(msg.ErrorMessageDetail);
                }
            }
        });
    }
