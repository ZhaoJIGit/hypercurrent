...
{
	public class BankMigrate : MigrateBase, IMigrate
	{
		public OperationResult SaveMigration(MContext ctx, MigrateDataModel model)
		{
			OperationResult operationResult = new OperationResult();
			List<CommandInfo> list = new List<CommandInfo>();
			List<BDBankAccountModel> modelData = base.DeSerializeObject<List<BDBankAccountModel>>(model.DataListJson);
		...
		public OperationResult DeleteMigration(MContext ctx, string migrationId)
		{
			...
			list.AddRange(ModelInfoManager.GetDeleteFlagCmd<BDBankAccountModel>(ctx, (from f in logList
