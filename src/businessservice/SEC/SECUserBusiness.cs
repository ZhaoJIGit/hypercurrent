...
";
        ...
        public DataGridJson<SECUserModel> GetUserList(MContext ctx, string email, string name, int pageIndex = 0, int pageSize = 10)
        {
            SqlWhere sqlWhere = new SqlWhere();
            if (!string.IsNullOrWhiteSpace(email))
            {
                sqlWhere.Like("MEmailAddress", email);
            ...
            return new DataGridJson<SECUserModel>
            {
        ...
        {
            OperationResult operationResult = new OperationResult();
            operationResult = new SECUserRepository().UpdateStatus(ctx, mItemId, status);
            //throw new NotImplementedException();
            return operationResult;
        }
    }
